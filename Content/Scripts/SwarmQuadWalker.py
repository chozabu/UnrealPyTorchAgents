...
state_dim += 6*6 #ang/lin vel
#state_dim += 8 # ground sensors
state_dim += 2 # contact sensors
state_dim += 2 # contact sensors

state_dim += 1 #target angle - should calc this in BP really!

#state_dim *= 2

action_dim = 14
action_dim += 14 # back legs
...
directory = "./NNModels/QuadWalker3d"  # save trained models
#filename = "TD3_BLIND"
loadpol = True
loadfilename = "TD3_QuadpedalWalker3dGlock2"
filename =     "TD3_QuadpedalWalker3dGlock3"
...
class TorchWalkerMinion:
    ...
    def tick(self, delta_time):
        ...
        # self.prev_obs = obs
        obs.append(self.get_target_angle())
        state = np.array(obs)
        ...
        reward += self.actor.body.get_up_vector().z
        ...
        reward -= self.actor.rfoot_touch*.5
        reward -= self.actor.lfoot_touch*.5
        reward -= self.actor.rfoot_touch2*.5
        reward -= self.actor.lfoot_touch2*.5

        #staying away from ground
        reward += self.actor.ground_dist
        reward += self.actor.ground_dist_2


        #punish when joints further from neutral position, reward when closer
        #print("")
        #print(self.actor.joint_obs)
        rv = 0
        ...
        if self.actor.ground_dist < .355:
            #print(state)
            self.actor.hit_body = False
            done = 1
            reward -= 70

        if self.actor.ground_dist_2 < .355:
            #print(state)
            self.actor.hit_body = False
            done = 1
            reward -= 70
